...
class AddressListContainer extends React.PureComponent {
  ...
  showThePopUpForPresideLocation() {
    Alert.alert("Epic", "Use precise location.", [
      {
        text: "OK",
        onPress: () => {
  ...
  getCurrentAddressLocation = () => {
    ...
    Geocoder.init("AIzaSyAtOyCW-Bi9xOaoLAvg7RiBmvL6FoAhU1s");
    ...
    return (
      ...
      >
        ...
        >
          ...
          {console.log(this.state.isPaymentStatus)}
          {/* PAYMENT SELECTION */}
          {this.isSelect ? (
            <RadioGroup
              color={EDColors.primary || EDColors.text}
          ...
          {this.state.isPaymentStatus == 0 && (
            <Text style={style.ProcessingTax}>Paypal Processing Fee : 5%</Text>
          )}
        </View>
  ...
  //onPaymentHandler
  onPaymentHandler = (action) => {
    this.setState({ isPaymentStatus: action });
  };
  ...
  onOrderDeliverySuccess = (objSuccess) => {
    ...
    if (objSuccess?.data?.status === RESPONSE_SUCCESS) {
      this.props.navigation.navigate("checkout", {
        ...
        payment_method: this.state.isPaymentStatus == 0 ? "paypal" : "cod",
    ...
    netStatus((isConnected) => {
      if (isConnected) {
        let objcheckDeliveryListParams = {
          ...
          is_delivered: 1,
          user_latitude: this.arrayAddresses[this.state.selectedIndex].latitude,
          user_longitude:
