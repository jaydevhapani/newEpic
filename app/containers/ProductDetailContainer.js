...
class ProductDetailContainer extends React.PureComponent {
  ...
  constructor(props) {
    super(props);
    this.quantity = 1;
    this.productDetail = JSON.parse(
      JSON.stringify(this.props.navigation.state.params.selectedProductDetails)
    );
    console.log("checkProvider ::: ", this.productDetail);
  }
        ...
        <EDRTLText
          ...
          title={
            value === null || value === undefined || value.trim().length === 0
              ? strings("productDetailsNew.notAvailable")
              : value
          }
        />
      </EDRTLView>
          ...
          <View style={{ flex: 1 }}>
            ...
            <ScrollView
              ...
              contentContainerStyle={{
                paddingBottom: this.props.cartCount > 0 ? 80 : 10,
              }}
            >
              <View style={style.mainContainer}>
                ...
                {this.productDetail.name !== undefined &&
                this.productDetail.name.trim().length > 0 ? (
                  <EDRTLText
                ...
                this.productDetail.isInStock === "1" ? (
                  <EDRTLView style={[style.commonView]}>
                    {this.productDetail.is_customize === "1" ? null : (
                      <EDRTLView>
                        <EDRTLText
                          style={style.productPrice}
                          title={
                            strings("productDetailsNew.price") +
                            ": " +
                        ...
                        {this.productDetail.offer_price !== "" ? (
                          <EDRTLText
                            style={style.productDecoratedPrice}
                            title={
                              this.props.currencySymbol +
                              funGetFrench_Curr(
                                this.productDetail.price,
                    ...
                    >
                      {this.state.quantity > 1 ? (
                        <MaterialIcon
                          onPress={this.buttonMinusPressed}
                          size={20}
                      ...
                      <View>
                        <Text style={style.qtystyle}>
                          {this.state.quantity}
                        </Text>
                      </View>

                      <MaterialIcon
                        onPress={this.buttonPlusPressed}
                        size={20}
                        color={EDColors.homeButtonColor}
                        name="add-circle"
                      />
                ...
                {/* PRODUCT DESCRIPTION */}
                {this.productDetail?.menu_detail !== null &&
                this.productDetail?.menu_detail.trim().length > 0 ? (
                  <EDRTLText
                    style={[style.titleText, { marginTop: 20 }]}
                ...
                {/* PRODUCT COMPOSITION */}
                {this.productDetail.composition !== null &&
                this.productDetail.composition.trim().length > 0 ? (
                  <EDRTLText
                ...
                {this.renderInfoComponent(
                  strings("productDetailsNew.isInStock"),
                  this.productDetail.isInStock === "0"
                    ? strings("buttonTitles.no")
                    : strings("buttonTitles.yes"),
                  style.alternateColor1
                )}

                {/* CATEGORY */}
                {this.productDetail.category_name != null &&
                  this.renderInfoComponent(
                    strings("productDetailsNew.category"),
                    this.productDetail.category_name,
                    style.alternateColor2
                  )}

                {/* BRAND */}
                {this.productDetail.brand_name != null &&
                  this.renderInfoComponent(
                    strings("productDetailsNew.brand"),
                    this.productDetail.brand_name,
                    style.alternateColor1
                  )}

                {/* FOOD TYPE */}
                {this.productDetail.food_type_name != undefined &&
                this.productDetail.food_type_name != null &&
            ...
            {/* ADD TO CART BUTTON */}
            {this.props.objStoreDetails.timings.closing.toLowerCase() ==
              "open" && this.productDetail.isInStock === "1" ? (
              <EDButton
  ...
  storeData = (data) => {
    ...
    if (cartdata != undefined && cartdata.items !== undefined) {
      if (cartdata.items.length > 0) {
        this.cartArray = cartdata.items;
        var repeatArray = this.cartArray.filter((item) => {
          return (
            item.menu_id === Category_data.menu_id &&
            Category_data.is_customize === "0"
          );
        });
        ...
        if (repeatArray.length > 0) {
          ...
          repeatArray[repeatArray.length - 1].quantity =
            repeatArray[repeatArray.length - 1].quantity + this.state.quantity;
        } else {
  ...
  buttonAddToCartPressed = () => {
      ...
      if (cartdata.items.length > 0) {
        ...
        if (
          this.cartArray.filter((item) => {
            return item.menu_id == this.productDetail.menu_id;
          }).length !== 0
        ) {
          if (
            this.cartArray.filter((item) => {
            ...
            currentQTY = this.cartArray.filter((item) => {
              return item.menu_id == this.productDetail.menu_id;
            })[0].quantity;
          }
        }
    ...
    if (this.productDetail.is_customize === "0") {
      if (
        this.productDetail.item_in_stock >=
        currentQTY + this.state.quantity
      ) {
        if (
          this.productDetail.item_max_quantity >=
          currentQTY + this.state.quantity
        ) {
      ...
      } else this.refs.toast.show(strings("generalNew.noMoreStock"), 1500);
    } else {
      this.props.cartDetail !== (undefined || null) &&
      this.props.cartDetail.items !== undefined &&
      this.props.cartDetail.items.some(
        (item) =>
          item.menu_id === this.productDetail.menu_id && item.quantity >= 1
      )
        ? this.setState({
...
export const style = StyleSheet.create({
  ...
  productName: {
    color: EDColors.productNameColor,
