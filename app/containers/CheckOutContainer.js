...
class CheckOutContainer extends React.PureComponent {
              ...
              <CheckOutBottomComponent
                ...
                title={
                  this.addToCartDict != undefined
                    ? this.props.currencySymbol +
                      funGetFrench_Curr(this.getTotalPrice(), 1, this.props.lan)
                    : ""
                }
  ...
  getTotalPrice() {
    let total_price;
    this.priceArray.map((item, index) => {
  ...
  componentDidMount() {
    console.log(
      "payment_method +++++ ",
      this.props.navigation.state.params.payment_method
    );
    ...
    netStatus((isConnected) => {
      if (isConnected) {
        ...
        var objaddToCartParams = {
          ...
          user_latitude: this.props.navigation.state.params.latitude,
          user_longitude: this.props.navigation.state.params.longitude,
          payment_method: this.props.navigation.state.params.payment_method,
        };
  ...
  onAddOrderSuccess = (objSuccess) => {
    ...
    if (objSuccess?.data) {
      ...
      if (status == RESPONSE_SUCCESS) {
        ...
        this.props.navigation.state.params.payment_method == "paypal"
