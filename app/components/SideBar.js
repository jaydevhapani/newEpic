...
class SideBar extends React.PureComponent {
  ...
  render() {
    ...
    let arrTemp =
      this.props.userDetails.UserID == 0
        ? this.setupSideMenuData_gest()
        : this.setupSideMenuData();
    let arraySideMenuData = arrTemp.concat(arrCMSPages);

    this.arrayFinalSideMenu = this.props.isLoggedIn
      ? arraySideMenuData.concat({
          isAsset: true,
          route: this.props.userDetails.UserID == 0 ? "login" : "SignOut",
          screenName:
            this.props.userDetails.UserID == 0
              ? "Sign In"
  ...
  setupSideMenuData_gest = () => {
    return [
      ...
      {
        route: "Share",
        screenName: strings("sidebarNew.share"),
        icon: "share",
      },
    ];
  };
